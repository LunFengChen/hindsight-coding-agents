...
function cursorFile(harness, sessionId) {
  return join7(tmpdir2(), `hindsight-${harness}`, `${sessionId}.retain.json`);
}
function fileCursorStore(harness) {
  return {
    read: (sessionId) => {
      try {
        return JSON.parse(readFileSync3(cursorFile(harness, sessionId), "utf8"));
      } catch {
        return void 0;
      }
    },
...
// src/core/retain-hook.ts
var HOST_DEADLINE_MARGIN_MS = 2e3;
async function buildRetain(args) {
  ...
  const decoded = args.lastAssistantMessage ? (args.readLastMessage ?? ((raw) => raw))(args.lastAssistantMessage) : "";
  ...
  const startTs = turns[0]?.timestamp ?? (/* @__PURE__ */ new Date()).toISOString();
  const t0 = Date.now();
  try {
    await retainLiveSession(client, sessionId, turns, startTs, harness, {
...
async function runRetainHook(spec, makeClient = (o) => new HindsightClient(o)) {
  if (process.env.HINDSIGHT_DISABLE_HOOKS) return;
  const hostDeadline = Date.now() + spec.hostTimeoutSec * 1e3 - HOST_DEADLINE_MARGIN_MS;
  let ev = {};
  try {
    ev = JSON.parse(readFileSync5(0, "utf8"));
  } catch {
    return;
  }
...
var runHarnessRetain = (harness) => runRetainHook(HOOK_HARNESSES[harness].retain);

// src/grok-stop-hook.ts
void runHarnessRetain("grok-build");
